...
def predict(x):
    ...
    for index, row in df.iterrows():
        ...
        if row['sex_female'] == 1 and row['pclass'] == 1 and row['who_child'] == 1:
            y = 0.9
        elif (row['sex_female'] == 1 and row['pclass'] == 1) or (row['sex_female'] == 1 and row['who_child'] == 1) or (row['pclass'] == 1 and row['who_child'] == 1):
            y = 0.7
        elif row['sex_female'] == 1 or row['pclass'] == 1 or row['who_child'] == 1:
            y = 0.5
        else:
            y = 0.1
...
Please note that this is a very simple rule-based approach and may not provide the most accurate predictions. For more accurate predictions, you may want to consider using machine learning algorithms.