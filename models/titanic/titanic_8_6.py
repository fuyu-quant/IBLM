...
def predict(x):
    ...
    for index, row in df.iterrows():
        ...
        deck_weight = 0.1 if row['deck_A'] else 0.2 if row['deck_B'] else 0.3 if row['deck_C'] else 0.4 if row['deck_D'] else 0.5 if row['deck_E'] else 0.6 if row['deck_F'] else 0.7
