...
def predict(x):
    ...
    for index, row in df.iterrows():
        ...
        deck_weight = 0.2 if row['deck_A'] else 0.1 if row['deck_B'] else 0.1 if row['deck_C'] else 0.1 if row['deck_D'] else 0.1 if row['deck_E'] else 0.1 if row['deck_F'] else 0.1 if row['deck_G'] else 0.2
        ...
        y = pclass_weight * age_weight * fare_weight * sex_weight * embarked_weight * alone_weight * adult_male_weight * class_weight * deck_weight * embark_town_weight
