...
def predict(x):
    ...
    for index, row in df.iterrows():
        ...
        # or when the absolute value of Feature_2 is greater than 0.5 and the absolute value of Feature_1 is less than 0.5.
        # We can use this observation to estimate the probability of the target being 1.

        if (abs(feature_1) > 0.5 and abs(feature_2) < 0.5) or (abs(feature_2) > 0.5 and abs(feature_1) < 0.5):
            y = 0.9
        else:
            y = 0.1
